...
class FinalResult(LevelResult):
    ...
    def __str__(self) -> str:
        return f"{'OK' if self.valid else 'DISQUALIFIED'}\n" \
               ...
               f"Time: {self.time:.6f} (SAT: {self.sat_time:.6f}, UNSAT: {self.unsat_time:.6f})\n" \
               f"TLEs: {self.tles} (SAT: {self.sat_tles}, UNSAT: {self.unsat_tles})\n" \
               f"Score: {self.score:.6f} (SAT: {self.sat_score:.6f}, UNSAT: {self.unsat_score:.6f})"
