...
import helpers
from result import TestResult, LevelResult, FinalResult
import validate
...
def submit_result(final_result: FinalResult) -> None:
    ...
    json_request["auth_token"] = os.environ["LEADERBOARD_AUTH_TOKEN"]
    json_request["user_id"] = os.environ["GITLAB_USER_LOGIN"]
    logger.debug(f"Posting request:\n{json_request}")
    pasresponse = requests.post(url, json=json_request, timeout=10)
